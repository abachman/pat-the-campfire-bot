          ...
          try {
            ...
            (new AdjectiveNoun).get(commit.after, function(release_name) {
              ...
              return campfire.room(process.env.campfire_bot_room, function(room) {
                var after, after_token, before, before_token, branch, c, compare_url, hash_token, link_to_commit, project, qty;
                try {
                  hash_token = /^(.{7})/;
                  ...
                  if (qty === 1) {
                    ...
                    return room.speak("[" + project + "/" + branch + "] current release: \"" + release_name + "\"");
                  }
                } catch (ex) {
