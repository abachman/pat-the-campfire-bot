(function() {
  var DomJS, Helpspot, HelpspotAPI, find_child_by_name, http, logger, populate_from_xml, querystring, _;
  ...
  populate_from_xml = function(data, obj, callback) {
    var domjs;
    domjs = new DomJS;
    return domjs.parse(data, function(err, dom) {
      if (err) {
        return obj;
      } else {
      ...
      return request.on('response', function(response) {
        ...
        return response.on('end', function() {
          ...
          try {
            ...
            return populate_from_xml(data, results, callback);
          } catch (e) {
            return console.log("Failed to parse Helpspot API response: " + e.message);
          }
        });
      });
    };
