(function() {
  ...
  Phrases = (function() {
    ...
    Phrases.prototype.add_phrase = function(pattern, phrase, message, room) {
      var body, modifiers, _ref2;
      _ref2 = this.get_isolated_pattern(pattern), body = _ref2.body, modifiers = _ref2.modifiers;
      return Phrase.findOne({
        ...
        var _phrase;
        if (phrase !== null) {
          room.speak("I already respond to /" + body + "/" + modifiers + ", sorry :(", this.logger);
          return;
        }
        _phrase = new Phrase({
          pattern: body,
        ...
        return _phrase.save(__bind(function(err, new_phrase) {
          return User.findOne({
            ...
            if (err) {
              room.speak("From now on, if anyone says " + body + ", I'll say \"" + phrase + "\"", this.logger);
            } else {
              room.speak("Thanks, " + (user.name.split(' ')[0]) + ", from now on if someone says " + body + ", I'll say \"" + phrase + "\"", this.logger);
            }
    ...
    Phrases.prototype.remove_phrase = function(pattern, room) {
      var body, modifiers, _ref2;
      _ref2 = this.get_isolated_pattern(pattern), body = _ref2.body, modifiers = _ref2.modifiers;
      return Phrase.findOne({
        pattern: body,
        ...
        if (err || phrase === null) {
          return room.speak("I couldn't find a phrase matching /" + body + "/" + modifiers);
        } else {
