...
Campfire.prototype.rooms = function(callback) {
  ...
  this.get('/rooms', function(data) {
    if (typeof data != 'object') {
      console.error('The call to Campfire.rooms failed. Verify that your credentials are correct.');
      return;
...
Campfire.prototype.put = function(path, body, callback) {
...
Campfire.prototype.request = function(method, path, body, callback) {
  ...
  if (method == 'POST' || method == 'PUT') {
    ...
    console.log("[Campfire request] making request with body: " + buf);
  ...
  var request = this.http.request(options, function(response) {
    ...
    if (typeof(callback) == 'function') {
      ...
      response.on('end', function() {
        try {
          if (data.trim() != '') {
            console.log("finished request to "+ options.path +", got " + data);
...
Campfire.Room.prototype.listen = function(callback) {
  ...
  campfire.http.request(options, function(response) {
    ...
    response.on('end', function () {
      console.log("[campfire.js] campfire dropped the connection, triggering disconnect event");
      self.events.emit('listen:disconnect');
      // process.exit(1);
    });
  }).end();
};

Campfire.Room.prototype.speak_or_play = function(text, callback) {
  if (/^\/play ([a-z]+)$/.test(text)) {
    var soundname = /^\/play ([a-z]+)$/.exec(text)[1];
    this.sound(soundname, callback);
  } else {
...
Campfire.Room.prototype.update = function(topic, callback) {
  this.put('', { room : { topic : topic } }, callback);
};

// undocumented streaming api request
Campfire.Room.prototype.ping = function(callback) {
  this.post('/ping', {}, callback);
};
...
Campfire.Room.prototype.put = function(path, body, callback) {
  this.campfire.put(this.path + path, body, callback);
};
