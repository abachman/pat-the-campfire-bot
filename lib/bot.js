(function() {
  var Campfire, Counter, EventEmitter, NODE_ENV, User, bot, debuglog, find_or_create_user, http, increment, instance, logger, plugins, port, server, store, track_message, util, _;
  http = require('http');
  ...
  instance.room(process.env.campfire_bot_room, function(room) {
    ...
    room.join(function() {
      ...
      ping_room = function() {
        ...
        return setInterval(ping_room, 60000 * 8);
      };
      return ping_room();
    });
  ...
  server = http.createServer(function(req, res) {
    ...
    return res.end("" + (bot.name || 'bot') + " <3s you\n");
  });
